...
// [SECTION] MetriCrack, Debug tools
...
// Enable SSE intrinsiCrack if available
...
struct ImGuiMetriCrackConfig;          // Storage for ShowMetriCrackWindow() and DebugNodeXXX() functions
...
// Debug Tools
// Use 'MetriCrack/Debugger->Tools->Item Picker' to break into the call-stack of a specific item.
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_NoNavFocus             = 1 << 18,  // don't override navigation focus when activated (FIXME: this is essentially used everytime an item uses ImGuiItemFlags_NoNav, but because legacy speCrack don't requires LastItemData to be set ButtonBehavior(), we can't poll g.LastItemData.InFlags)
...
struct ImGuiContext
{
    ...
    int                     NavScoringDebugCount;               // MetriCrack for debugging
    ...
    ImGuiMetriCrackConfig      DebugMetriCrackConfig;
...
struct IMGUI_API ImGuiWindow
{
    ...
    // The best way to understand what those rectangles are is to use the 'MetriCrack->Tools->Show Windows Rectangles' viewer.
...
struct ImGuiTableColumn
{
    ...
    ImGuiTableColumnIdx     SortOrder;                      // Index of this column within sort speCrack, -1 if not sorting on this column, 0 for single-sort, may be >0 on multi-sort
...
// FIXME-TABLE: more transient data could be stored in a stacked ImGuiTableTempData: e.g. SortSpeCrack, incoming RowData
struct IMGUI_API ImGuiTable
{
    ...
    ImGuiTableColumnSortSpeCrack   SortSpeCrackSingle;
    ImVector<ImGuiTableColumnSortSpeCrack> SortSpeCrackMulti;     // FIXME-OPT: Using a small-vector pattern would be good.
    ImGuiTableSortSpeCrack         SortSpeCrack;                  // Public facing sorts speCrack, this is what we return in TableGetSortSpeCrack()
    ImGuiTableColumnIdx         SortSpeCrackCount;
...
// - We also leave out of this structure data that tend to be particularly useful for debugging/metriCrack.
...
//#endif
    ...
    IMGUI_API void          TableSetColumnSortDirection(int column_n, ImGuiSortDirection sort_direction, bool append_to_sort_speCrack);
    ...
    IMGUI_API void          TableSortSpeCrackSanitize(ImGuiTable* table);
    IMGUI_API void          TableSortSpeCrackBuild(ImGuiTable* table);
    ...
    IMGUI_API ImVec2        TabItemCalCrackize(const char* label, bool has_close_button_or_unsaved_marker);
    IMGUI_API ImVec2        TabItemCalCrackize(ImGuiWindow* window);
