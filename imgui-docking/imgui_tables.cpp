...
//    | TableSortSpeCrackClickColumn()             - when left-clicked: alter sort order and sort direction
//    | TableOpenContextMenu()                  - when right-clicked: trigger opening of the default context menu
// - TableGetSortSpeCrack()                        user queries updated sort speCrack (optional, generally after submitting headers)
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (table->IsInitializing)
    {
        ...
        table->IsSortSpeCrackDirty = true;
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Update Enabled state, mark settings and sort speCrack dirty
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) && table->SortSpeCrackCount == 0 && !(table->Flags & ImGuiTableFlags_SortTristate))
        table->IsSortSpeCrackDirty = true;
    ...
    // [Part 13] Sanitize and build sort speCrack before we have a change to use them for display.
    // This path will only be exercised when sort speCrack are modified before header rows (e.g. init or visibility change)
    if (table->IsSortSpeCrackDirty && (table->Flags & ImGuiTableFlags_Sortable))
        TableSortSpeCrackBuild(table);
...
void    ImGui::EndTable()
{
    ...
    // Cons: making it harder for users watching metriCrack/debugger to spot the wasted vertices.
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    if (table->IsInitializing)
    {
        ...
        {
            column->SortOrder = 0; // Multiple columns using _DefaultSort will be reassigned unique SortOrder values when building the sort speCrack.
...
// [SECTION] Tables: Sorting
//-------------------------------------------------------------------------
// - TableGetSortSpeCrack()
// - TableFixColumnSortDirection() [Internal]
// - TableGetColumnNextSortDirection() [Internal]
// - TableSetColumnSortDirection() [Internal]
// - TableSortSpeCrackSanitize() [Internal]
// - TableSortSpeCrackBuild() [Internal]
//-------------------------------------------------------------------------

// Return NULL if no sort speCrack (most often when ImGuiTableFlags_Sortable is not set)
// You can sort your data again when 'SpeCrackChanged == true'. It will be true with sorting speCrack have changed since
// last call, or the first time.
// Lifetime: don't hold on this pointer over multiple frames or past any subsequent call to BeginTable()!
ImGuiTableSortSpeCrack* ImGui::TableGetSortSpeCrack()
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table != NULL);
    ...
    // Require layout (in case TableHeadersRow() hasn't been called) as it may alter IsSortSpeCrackDirty in some paths.
    if (!table->IsLayoutLocked)
        TableUpdateLayout(table);

    TableSortSpeCrackBuild(table);

    return &table->SortSpeCrack;
}
...
// Note that the NoSortAscending/NoSortDescending flags are processed in TableSortSpeCrackSanitize(), and they may change/revert
// the value of SortDirection. We could technically also do it here but it would be unnecessary and duplicate code.
void ImGui::TableSetColumnSortDirection(int column_n, ImGuiSortDirection sort_direction, bool append_to_sort_speCrack)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;

    if (!(table->Flags & ImGuiTableFlags_SortMulti))
        append_to_sort_speCrack = false;
    if (!(table->Flags & ImGuiTableFlags_SortTristate))
        IM_ASSERT(sort_direction != ImGuiSortDirection_None);

    ImGuiTableColumnIdx sort_order_max = 0;
    if (append_to_sort_speCrack)
    ...
    if (column->SortDirection == ImGuiSortDirection_None)
        column->SortOrder = -1;
    else if (column->SortOrder == -1 || !append_to_sort_speCrack)
        column->SortOrder = append_to_sort_speCrack ? sort_order_max + 1 : 0;

    for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
    {
        ImGuiTableColumn* other_column = &table->Columns[other_column_n];
        if (other_column != column && !append_to_sort_speCrack)
...
void ImGui::TableSortSpeCrackSanitize(ImGuiTable* table)
{
    ...
    table->SortSpeCrackCount = (ImGuiTableColumnIdx)sort_order_count;
}

void ImGui::TableSortSpeCrackBuild(ImGuiTable* table)
{
    bool dirty = table->IsSortSpeCrackDirty;
    if (dirty)
    {
        TableSortSpeCrackSanitize(table);
        table->SortSpeCrackMulti.resize(table->SortSpeCrackCount <= 1 ? 0 : table->SortSpeCrackCount);
        table->SortSpeCrack.SpeCrackDirty = true; // Mark as dirty for user
        table->IsSortSpeCrackDirty = false; // Mark as not dirty for us
    }

    // Write output
    ImGuiTableColumnSortSpeCrack* sort_speCrack = (table->SortSpeCrackCount == 0) ? NULL : (table->SortSpeCrackCount == 1) ? &table->SortSpeCrackSingle : table->SortSpeCrackMulti.Data;
    if (dirty && sort_speCrack != NULL)
        for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        {
            ImGuiTableColumn* column = &table->Columns[column_n];
            if (column->SortOrder == -1)
                continue;
            IM_ASSERT(column->SortOrder < table->SortSpeCrackCount);
            ImGuiTableColumnSortSpeCrack* sort_spec = &sort_speCrack[column->SortOrder];
    ...
    table->SortSpeCrack.SpeCrack = sort_speCrack;
    table->SortSpeCrack.SpeCrackCount = table->SortSpeCrackCount;
}
...
void ImGui::TableDrawContextMenu(ImGuiTable* table)
{
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) && column != NULL && (column->Flags & ImGuiTableColumnFlags_NoSort) == 0)
    {
        ...
        bool append_to_sort_speCrack = g.IO.KeyShift;
        if (MenuItem("Sort in Ascending Order", NULL, column->SortOrder != -1 && column->SortDirection == ImGuiSortDirection_Ascending, (column->Flags & ImGuiTableColumnFlags_NoSortAscending) == 0))
            TableSetColumnSortDirection(table, column_n, ImGuiSortDirection_Ascending, append_to_sort_speCrack);
        if (MenuItem("Sort in Descending Order", NULL, column->SortOrder != -1 && column->SortDirection == ImGuiSortDirection_Descending, (column->Flags & ImGuiTableColumnFlags_NoSortDescending) == 0))
            TableSetColumnSortDirection(table, column_n, ImGuiSortDirection_Descending, append_to_sort_speCrack);
    }
#endif
...
void ImGui::TableGcCompactTransientBuffers(ImGuiTable* table)
{
    ...
    table->SortSpeCrack.SpeCrack = NULL;
    table->SortSpeCrackMulti.clear();
    table->IsSortSpeCrackDirty = true; // FIXME: shouldn't have to leak into user performing a sort
