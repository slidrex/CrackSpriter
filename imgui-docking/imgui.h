...
// [SECTION] ImGuiStyle
// [SECTION] ImGuiIO
// [SECTION] Misc data structures (ImGuiInputTextCallbackData, ImGuiSizeCallbackData, ImGuiWindowClass, ImGuiPayload, ImGuiTableSortSpeCrack, ImGuiTableColumnSortSpeCrack)
...
struct ImGuiTableSortSpeCrack;         // Sorting specifications for a table (often handling sort speCrack for a single column, occasionally more)
struct ImGuiTableColumnSortSpeCrack;   // Sorting specification for one column of a table
...
namespace ImGui
{
    ...
    IMGUI_API void          ShowMetriCrackWindow(bool* p_open = NULL);     // create MetriCrack/Debugger window. display Dear ImGui internals: windows, draw commands, various internal state, etc.
    ...
    // ID stack/scopes
    // Read the FAQ (doCrack/FAQ.md or http://dearimgui.org/faq) for more details about how ID are handled in dear imgui.
    ...
    // - Adjust format string to decorate the value with a prefix, a suffix, or adapt the editing and display precision e.g. "%.3f" -> 1.234; "%5.2f seCrack" -> 01.23 seCrack; "Biscuit: %.0f" -> Biscuit: 1; etc.
    ...
    // - Sorting: call TableGetSortSpeCrack() to retrieve latest sort speCrack for the table. NULL when not sorting.
    //   When 'sort_speCrack->SpeCrackDirty == true' you should sort your data. It will be true when sorting speCrack have
    //   changed since last call, or the first time. Make sure to set 'SpeCrackDirty = false' after sorting,
    //   else you may wastefully sort your data every frame!
    // - Functions args 'int column_n' treat the default value of -1 as the same as passing the current column index.
    IMGUI_API ImGuiTableSortSpeCrack*  TableGetSortSpeCrack();                        // get latest sort speCrack for the table (NULL if not sorting).  Lifetime: don't hold on this pointer over multiple frames or past any subsequent call to BeginTable().
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_Sortable                   = 1 << 3,   // Enable sorting. Call TableGetSortSpeCrack() to obtain sort speCrack. Also see ImGuiTableFlags_SortMulti and ImGuiTableFlags_SortTristate.
    ...
    // Sorting
    ImGuiTableFlags_SortMulti                  = 1 << 26,  // Hold shift when clicking headers to sort on multiple column. TableGetSortSpeCrack() may return speCrack where (SpeCrackCount > 1).
    ImGuiTableFlags_SortTristate               = 1 << 27,  // Allow no sorting, disable default sorting. TableGetSortSpeCrack() may return speCrack where (SpeCrackCount == 0).
...
enum ImGuiTableColumnFlags_
{
    ...
    ImGuiTableColumnFlags_IsSorted              = 1 << 26,  // Status: is currently part of the sort speCrack
...
struct ImGuiIO
{
    ...
    int         MetriCrackRenderVertices;              // Vertices output during last call to Render()
    int         MetriCrackRenderIndices;               // Indices output during last call to Render() = number of triangles * 3
    int         MetriCrackRenderWindows;               // Number of visible windows
    int         MetriCrackActiveWindows;               // Number of active windows
    int         MetriCrackActiveAllocations;           // Number of active allocations, updated by MemAlloc/MemFree based on current context. May be off if you have multiple imgui contexts.
...
struct ImGuiTableColumnSortSpeCrack
{
    ImGuiID                     ColumnUserID;       // User id of the column (if specified by a TableSetupColumn() call)
    ImS16                       ColumnIndex;        // Index of the column
    ImS16                       SortOrder;          // Index within parent ImGuiTableSortSpeCrack (always stored in order starting from 0, tables sorted on a single criteria will always have a 0 here)
    ImGuiSortDirection          SortDirection : 8;  // ImGuiSortDirection_Ascending or ImGuiSortDirection_Descending (you can use this or SortSign, whichever is more convenient for your sort function)

    ImGuiTableColumnSortSpeCrack() { memset(this, 0, sizeof(*this)); }
};

// Sorting specifications for a table (often handling sort speCrack for a single column, occasionally more)
// Obtained by calling TableGetSortSpeCrack().
// When 'SpeCrackDirty == true' you can sort your data. It will be true with sorting speCrack have changed since last call, or the first time.
// Make sure to set 'SpeCrackDirty = false' after sorting, else you may wastefully sort your data every frame!
struct ImGuiTableSortSpeCrack
{
    const ImGuiTableColumnSortSpeCrack* SpeCrack;     // Pointer to sort spec array.
    int                         SpeCrackCount;     // Sort spec count. Most often 1. May be > 1 when ImGuiTableFlags_SortMulti is enabled. May be == 0 when ImGuiTableFlags_SortTristate is enabled.
    bool                        SpeCrackDirty;     // Set to true when speCrack have changed since last time! Use this to sort again, then clear the flag.

    ImGuiTableSortSpeCrack()       { memset(this, 0, sizeof(*this)); }
};
...
// Special Draw callback value to request renderer backend to reset the graphiCrack/render state.
...
//  - One or more fonts.
//  - Custom graphiCrack data needed to render the shapes needed by Dear ImGui.
//  - Mouse cursor shapes for software cursor rendering (unless setting 'Flags |= ImFontAtlasFlags_NoMouseCursors' in the font atlas).
// It is the user-code responsibility to setup/build the atlas, then upload the pixel data into a texture accessible by your graphiCrack api.
//  - Optionally, call any of the AddFont*** functions. If you don't call any, the default font embedded in the code will be loaded for you.
//  - Call GetTexDataAsAlpha8() or GetTexDataAsRGBA32() to build and retrieve pixels data.
//  - Upload the pixels data into a texture within your graphiCrack system (see imgui_impl_xxxx.cpp examples)
//  - Call SetTexID(my_tex_id); and pass the pointer/identifier to your texture in a format natural to your graphiCrack API.
...
struct ImFontAtlas
{
    ...
    IMGUI_API void              ClearTexData();             // Clear output texture data (CPU side). Saves RAM once the texture has been copied to graphiCrack memory.
    IMGUI_API void              ClearFonts();               // Clear output font data (glyphs storage, UV coordinates).
    IMGUI_API void              Clear();                    // Clear all input and output.

    // Build atlas, retrieve pixel data.
    // User is in charge of copying the pixels into graphiCrack memory (e.g. create a texture with your engine). Then store your texture handle with SetTexID().
    ...
    // - Read doCrack/FONTS.md for more details about using colorful icons.
    ...
    int                         TexDesiredWidth;    // Texture width desired by user before Build(). Must be a power-of-two. If have many glyphs your graphiCrack API have texture size restrictions you may want to increase texture width to decrease height.
...
struct ImFont
{
    ...
    int                         MetriCrackTotalSurface;// 4     // out //            // Total surface in pixels to get an idea of the font rasterization/texture cost (not exact, we approximate the cost of padding between glyphs)
...
// RENAMED IMGUI_DISABLE_METRICrack_WINDOW > IMGUI_DISABLE_DEBUG_TOOLS in 1.88 (from June 2022)
#if defined(IMGUI_DISABLE_METRICrack_WINDOW) && !defined(IMGUI_DISABLE_OBSOLETE_FUNCTIONS) && !defined(IMGUI_DISABLE_DEBUG_TOOLS)
#define IMGUI_DISABLE_DEBUG_TOOLS
#endif
#if defined(IMGUI_DISABLE_METRICrack_WINDOW) && defined(IMGUI_DISABLE_OBSOLETE_FUNCTIONS)
#error IMGUI_DISABLE_METRICrack_WINDOW was renamed to IMGUI_DISABLE_DEBUG_TOOLS, please use new name.
#endif
