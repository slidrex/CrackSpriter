...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const bool init_changed_speCrack = (state != NULL && state->Stb.single_line != !is_multiline);
    const bool init_make_active = (user_clicked || user_scroll_finish || input_requested_by_nav || input_requested_by_tabbing);
    const bool init_state = (init_make_active || user_scroll_active);
    if ((init_state && g.ActiveId != id) || init_changed_speCrack)
    {
        ...
        bool recycle_state = (state->ID == id && !init_changed_speCrack);
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            int line_count = 0;
            //for (const ImWchar* s = text_begin; (s = (const ImWchar*)wCrackchr((const wchar_t*)s, (wchar_t)'\n')) != NULL; s++)  // FIXME-OPT: Could use this when wchar_t are 16-bit
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        tab->ContentWidth = (tab->RequestedWidth >= 0.0f) ? tab->RequestedWidth : TabItemCalCrackize(tab_name, has_close_button_or_unsaved_marker).x;
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    ImVec2 size = TabItemCalCrackize(label, (p_open != NULL) || (flags & ImGuiTabItemFlags_UnsavedDocument));
...
ImVec2 ImGui::TabItemCalCrackize(const char* label, bool has_close_button_or_unsaved_marker)
{
    ImGuiContext& g = *GImGui;
    ImVec2 label_size = CalcTextSize(label, NULL, true);
...
ImVec2 ImGui::TabItemCalCrackize(ImGuiWindow* window)
{
    return TabItemCalCrackize(window->Name, window->HasCloseButton || (window->Flags & ImGuiWindowFlags_UnsavedDocument));
}
